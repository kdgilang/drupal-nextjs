...
export default props => {
  ...
  const baseUrl = get(props,'baseUrl');
  const instagram_token = get(props,'instagram_token_val');
  const pageTitle = get(props,'data.node_basic_data.title');
  const showPageTitle = get(props, "data.node_basic_data.path.alias")!=="/home";
  const subhead = (!landingPageCheck) ? get(props,'data.node_basic_data.field_subhead') : null;
  const shortTitle = (!landingPageCheck) ? get(props,'data.node_basic_data.field_short_title') : null;
  const breadcrumbData = get(props,'data.breadcrumb');
  const pageBody = (!landingPageCheck) ? get(props,'data.node_basic_data.body.value') : null;
    ...
    <Fragment>
      ...
      <div id="main-wrapper" className={landingPageCheck ? "ezcontent-main-wrapper landing-page-holder" : "ezcontent-main-wrapper"}>
        <div className={landingPageCheck ? "mt-4 mb-4" : "container mt-4 mb-4"}>
          ...
          {
            pageBody && <div className="entry-content" dangerouslySetInnerHTML={{ __html: pageBody }}/>
          }
          {block}
          {Author}
        </div>
